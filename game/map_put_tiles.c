...
/*   Updated: 2022/01/18 22:39:01 by nbenhado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	put_wall(t_mlx *vars, int x, int y)
{
	...
	if (!(vars->maps->img = mlx_xpm_file_to_image(vars->mlx, vars->maps->r_path, &vars->maps->img_width, &vars->maps->img_height)))
		return (0);
...
int	put_player(t_mlx *vars, int x, int y)
{
	...
	if (!(vars->player->img = mlx_xpm_file_to_image(vars->mlx, vars->player->r_path, &vars->player->img_width, &vars->player->img_height)))
		return (0);
